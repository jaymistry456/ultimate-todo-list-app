...
public class TaskListStarredView extends Pane {

    public TaskListStarredView() {
        Main.taskListModel.starredTaskListProperty().addListener(new ListChangeListener<TaskWidget>() {
            @Override
    ...
    public void displayList() throws IOException {
        ...
        for (TaskWidget t : Main.taskListModel.starredTaskListProperty) {
            ...
            important.setOnAction(e -> {
                ...
                for(TaskWidget task: Main.taskListModel.todaysTaskListProperty) {
                    if(task.getTaskName()==t.getTaskName() && task.getTaskDescription()==t.getTaskDescription()) {
                        task.setImportant(false);
                        break;
                    }
                }
                for(TaskWidget task: Main.taskListModel.upcomingTaskListProperty) {
                    if(task.getTaskName()==t.getTaskName() && task.getTaskDescription()==t.getTaskDescription()) {
                        task.setImportant(false);
                        break;
                    }
